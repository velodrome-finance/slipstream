...
abstract contract UniswapV3PoolSwapAllStakedNoUnstakeFeeTest is UniswapV3PoolSwapTests {
    ...
    function setUp() public virtual override {
        ...
        string memory path = string(
            abi.encodePacked(
                root,
                "/test/unit/concrete/UniswapV3Pool/swapTests/swapAllStakedNoUnstakedFee/swap_assert_all_staked.json"
            )
        );
    ...
    function burnPosition() internal override {
        ...
        for (uint256 i = 0; i < positionsLength; i++) {
            ...
            UniswapV3Pool(poolSetup.pool).burn(
                position.tickLower, position.tickUpper, position.liquidity, poolSetup.gauge
            );
