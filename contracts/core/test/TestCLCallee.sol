...
contract TestCLCallee is ICLMintCallback, ICLSwapCallback, ICLFlashCallback {
    ...
    function uniswapV3MintCallback(uint256 amount0Owed, uint256 amount1Owed, bytes calldata data) external override {
        ...
        if (amount0Owed > 0) {
            IERC20Minimal(ICLPool(msg.sender).token0()).transferFrom(sender, msg.sender, amount0Owed);
        }
        if (amount1Owed > 0) {
            IERC20Minimal(ICLPool(msg.sender).token1()).transferFrom(sender, msg.sender, amount1Owed);
        }
    }
    ...
    function flash(address pool, address recipient, uint256 amount0, uint256 amount1, uint256 pay0, uint256 pay1)
        external
    {
        ICLPool(pool).flash(recipient, amount0, amount1, abi.encode(msg.sender, pay0, pay1));
    }
    ...
    function uniswapV3FlashCallback(uint256 fee0, uint256 fee1, bytes calldata data) external override {
        ...
        if (pay0 > 0) IERC20Minimal(ICLPool(msg.sender).token0()).transferFrom(sender, msg.sender, pay0);
        if (pay1 > 0) IERC20Minimal(ICLPool(msg.sender).token1()).transferFrom(sender, msg.sender, pay1);
    }
}
