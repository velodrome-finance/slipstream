...
contract TestCLRouter is ICLSwapCallback {
    ...
    function uniswapV3SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata data) public override {
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid = amount0Delta > 0 ? ICLPool(msg.sender).token0() : ICLPool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == ICLPool(pools[0]).token1();
            ICLPool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(ICLPool(msg.sender).token0()).transferFrom(payer, msg.sender, uint256(amount0Delta));
            } else {
                IERC20Minimal(ICLPool(msg.sender).token1()).transferFrom(payer, msg.sender, uint256(amount1Delta));
            }
        }
    }
}
