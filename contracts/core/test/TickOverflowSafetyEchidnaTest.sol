        ...
        while (tick != target) {
            if (tick < target) {
                if (ticks[tick + 1].liquidityGross > 0) {
                    ticks.cross(tick + 1, feeGrowthGlobal0X128, feeGrowthGlobal1X128, 0, 0, uint32(block.timestamp), 0);
                }
                tick++;
            } else {
                if (ticks[tick].liquidityGross > 0) {
                    ticks.cross(tick, feeGrowthGlobal0X128, feeGrowthGlobal1X128, 0, 0, uint32(block.timestamp), 0);
                }
                tick--;
            }
        }
